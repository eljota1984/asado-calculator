...
export default function ResumenPage() {
  ...
  const compraSugerida = useMemo(() => {
    ...
    return seleccionados.map((producto) => {
      ...
      let cantidadSugerida = Math.ceil(kilosNecesariosBrutos / pesoPromedio);
      let kilosCompraAprox = cantidadSugerida * pesoPromedio;
      let unidadSugerida = "pieza(s)";

      if (producto.tipo === "vacuno" || producto.tipo === "cerdo") {
        cantidadSugerida = Math.ceil(kilosNecesariosBrutos);
        kilosCompraAprox = cantidadSugerida;
        unidadSugerida = "kg";
      }

      if (producto.tipo === "pollo") {
        if (nombreProducto.includes("pollo entero")) {
          cantidadSugerida = Math.ceil(kilosNecesariosBrutos / pesoPromedio);
          kilosCompraAprox = cantidadSugerida * pesoPromedio;
          unidadSugerida = "pollo(s) entero(s)";
        } else if (
          nombreProducto.includes("filetito") ||
          nombreProducto.includes("deshuesada")
        ) {
          cantidadSugerida = Math.ceil(kilosNecesariosBrutos);
          kilosCompraAprox = cantidadSugerida;
          unidadSugerida = "kg";
        } else if (nombreProducto.includes("pechuga")) {
          const mediasPechugas = Math.ceil(kilosNecesariosBrutos / 0.25);
          cantidadSugerida = mediasPechugas;
          kilosCompraAprox = mediasPechugas * 0.25;
          unidadSugerida =
            mediasPechugas === 1 ? "media pechuga" : "medias pechugas";
        } else if (
          nombreProducto.includes("trutro") ||
          nombreProducto.includes("alita")
        ) {
          cantidadSugerida = Math.ceil(kilosNecesariosBrutos / 0.18);
          kilosCompraAprox = cantidadSugerida * 0.18;
          unidadSugerida = "pieza(s)";
        } else {
          cantidadSugerida = Math.ceil(kilosNecesariosBrutos);
          kilosCompraAprox = cantidadSugerida;
          unidadSugerida = "kg";
      ...
      if (producto.tipo === "embutidos") {
        cantidadSugerida = Math.ceil(totalPersonas * 2);
        kilosCompraAprox = cantidadSugerida * 0.1;
        unidadSugerida = "unidad(es)";
      }
  ...
  }, [seleccionados, kilosTotales, totalPersonas]);
  ...
  const generarPDF = () => {
    ...
    compraSugerida.forEach((item) => {
      ...
      doc.text(
        `Sugerencia: ${item.cantidadSugerida} ${item.unidadSugerida}`,
        25,
        y
      );
      y += 6;
